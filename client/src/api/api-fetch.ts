...
export function apiFetch<T = any>({ctx, authToken, method, url, body, neverCancel}: ApiFetchRequest): Promise<ApiResponse<T>> {
    ...
    return _fetch(input, init)
        .then((httpRes): Promise<ApiResponse<T>> => {
            const statusClass = Math.floor(httpRes.status / 100);
            if (statusClass === 2) {
                return apiJson(httpRes).then((result) => ({success: true, result}));
            } else if (statusClass === 4) {
                return apiJson(httpRes).then((error) => {
...
function apiJson(httpRes: Response) {
